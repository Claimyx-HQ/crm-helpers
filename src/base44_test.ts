...
Deno.test('withRetry: jitter de-correlates across INDEPENDENT requests', async () => {
  // 20 sequential samples, EACH WITH ITS OWN RetryState — this is the
  // cross-request case (different Deno.serve invocations, different
  // workspace functions firing concurrently, the browser polling on a
  // tight interval) where shared cooldown does NOT apply and jitter is
  // the only mechanism preventing realignment.
  //
  // What this test does NOT cover, by design:
  //   Parallel callers sharing a SINGLE RetryState (the within-request
  //   case, e.g. siblings inside one inChunks loop). Those callers
  //   intentionally realign on `state.rateLimitUntil` — that's the
  //   shared-cooldown feature, not a bug. Only one caller's exponential
  //   backoff is in flight at a time and the others wait at the shared
  //   floor; this is the intended behavior to stop within-request
  //   sibling hammering. A test asserting "non-realignment" with shared
  //   state would FAIL by design.
  //
  // Assertion: max − min > 400ms proves the draws are independent
  // rather than identical, which is what prevents the thundering-herd
  // realignment across independent requests in production.
